...
const globals = ['header', 'footer', 'festival'];
export const seed = async ({ payload, req, }) => {
    ...
    await Promise.all(globals.map((global) => payload.updateGlobal({
        slug: global,
    ...
    await Promise.all(allCollections.map((collection) => payload.db.deleteMany({ collection, req, where: {} })));
    ...
    for (let i = 0; i < events.length; i++) {
        const event = events[i]; // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    for (let i = 0; i < clubs.length; i++) {
        const club = clubs[i]; // eslint-disable-line @typescript-eslint/no-explicit-any
        const mediaId = i === 0 ? mediaLight.id : mediaDark.id;
        await payload.create({
            collection: 'clubs',
            data: {
    ...
    for (const post of posts) {
        await payload.create({
            collection: 'posts',
            data: post, // eslint-disable-line @typescript-eslint/no-explicit-any
        });
    }
